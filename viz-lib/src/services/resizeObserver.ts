...
interface Watched {
  /** Undefined until the first measurement, so the first pass always fires. */
  width?: number;
  height?: number;
  callbacks: Set<(node: any) => void>;
}

const items = new Map<any, Watched>();
...
export default function observe(node: any, callback: (node: any) => void) {
  if (!node) {
    return () => {};
  }
