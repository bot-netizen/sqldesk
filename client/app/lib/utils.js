...
// Display-only sentence casing for user-entered names. Only the first
// character is touched: title-casing every word would rewrite "MRR waterfall"
// and "ETL freshness" into something their authors did not write.
...
// Row counts are denormalised onto the result at write time. Results stored
// before that column existed have none, and NULL means "we do not know",
// which is not the same as "zero rows" — so it renders as a dash.
...
// Query runtimes are frequently sub-second, where durationHumanize rounds the
// count and reports "0 seconds". This keeps small values legible and defers to
// durationHumanize once the value is large enough for its units to read well.
export function formatRuntime(seconds) {
  ...
  return durationHumanize(seconds);
}
