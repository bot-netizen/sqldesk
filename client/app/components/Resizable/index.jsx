...
export default function Resizable({ toggleShortcut, direction, sizeAttribute, children }) {
  ...
  const toggle = useCallback(() => {
    ...
    // Was a d3 transition; CSS does the same easing off the main thread, and it
    // was the last thing in this file keeping d3 v3 installed.
    element.style.setProperty(sizeAttribute, savedSize.current || "0px");
    element.style.transition = `${sizeAttribute} 200ms ease-in-out`;
    const clearTransition = () => {
      element.style.transition = "";
      element.removeEventListener("transitionend", clearTransition);
    };
    element.addEventListener("transitionend", clearTransition);
    // Read back so the starting size is committed before the target is set,
    // otherwise the browser collapses both writes and nothing animates.
    void element.offsetHeight;
    element.style.setProperty(sizeAttribute, targetSize);
  ...
  const resizeEventHandlers = useMemo(
    () => ({
      ...
      onResize: (unused, data) => {
        ...
        elementRef.current.style.setProperty(sizeAttribute, `${data.size[sizeProp]}px`);
