...
export default function SchemaBrowser({
  ...
  const isFileUploadSource = !!dataSource && dataSource.type === "duckdb";
  ...
  if (schema.length === 0 && !isLoading && !isFileUploadSource) {
    return null;
  }
  ...
  function confirmUpload() {
    ...
    setUploading(true);
    Uploads.upload(dataSource.id, pendingFile, uploadName.trim())
      .then((uploaded) => {
        ...
        {isFileUploadSource && (
          <React.Fragment>
            <input
              ...
              data-test="SchemaBrowserFileInput"
            />
            <Tooltip title="Upload File">
              <Button className="m-l-5" onClick={() => fileInputRef.current.click()} data-test="SchemaBrowserUpload">
            ...
            <Modal
              ...
              <label htmlFor="schemaBrowserUploadName">Name</label>
              <Input
                id="schemaBrowserUploadName"
                className="m-t-5"
                value={uploadName}
                onChange={(e) => setUploadName(e.target.value)}
                onPressEnter={confirmUpload}
                autoFocus
                data-test="SchemaBrowserUploadNameInput"
              />
              {pendingFile && (
