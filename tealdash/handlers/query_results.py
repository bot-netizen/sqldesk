...
from tealdash.handlers.base import BaseResource, get_object_or_404, record_event
...
from tealdash.serializers import (
    serialize_job,
...
from tealdash.tasks.queries import enqueue_query
...
def run_query(query, parameters, data_source, query_id, should_apply_auto_limit, max_age=0):
    ...
    if query_result:
        # A pre-encoded body, not a dict: returning a dict would send the payload
        # back through json_dumps, which is the cost this path exists to avoid.
        # Flask-RESTful passes Response objects through its representation.
        return current_app.response_class(
            serialize_query_result_json(query_result, current_user.is_api_user()),
...
class QueryResultResource(BaseResource):
    ...
    def get(self, query_id=None, query_result_id=None, filetype="json"):
        ...
        if query_result_id:
            query_result = get_object_or_404(
                models.QueryResult.get_by_id_and_org_deferred_data, query_result_id, self.current_org
            )
        ...
        if query_id is not None:
            ...
            if query_result is None and query is not None and query.latest_query_data_id is not None:
                query_result = get_object_or_404(
                    models.QueryResult.get_by_id_and_org_deferred_data,
    ...
    def make_json_response(query_result):
        data = serialize_query_result_json(query_result, is_api_user=False)
