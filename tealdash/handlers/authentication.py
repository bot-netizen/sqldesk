...
from tealdash import __version__, limiter, models, settings
from tealdash.authentication import current_org, get_login_url, get_next_path
from tealdash.authentication.account import (
    send_password_reset_email,
    send_user_disabled_email,
    send_verify_email,
    validate_token,
)
from tealdash.handlers import routes
from tealdash.handlers.base import json_response, org_scoped_rule
from tealdash.version_check import get_latest_version
...
def render_token_login_page(template, org_slug, token, invite):
    ...
    google_auth_url = get_google_auth_url(url_for("tealdash.index", org_slug=org_slug))
...
def verify(token, org_slug=None):
    ...
    next_url = url_for("tealdash.index", **template_context)
...
def login(org_slug=None):
    ...
    index_url = url_for("tealdash.index", org_slug=org_slug)
...
def base_href():
    if settings.MULTI_ORG:
        base_href = url_for("tealdash.index", _external=True, org_slug=current_org.slug)
    else:
        base_href = url_for("tealdash.index", _external=True)
