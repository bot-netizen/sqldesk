...
from tealdash import __version__, create_app, rq_redis_connection, settings
from tealdash.cli import (
    data_sources,
    database,
    groups,
    organization,
...
def manager():
    """Management script for Tealdash"""
...
@manager.command()
def check_settings():
    """Show the settings as Tealdash sees them (useful for debugging)."""
...
def send_test_mail(email=None):
    ...
    from tealdash import mail

    if email is None:
        email = settings.MAIL_DEFAULT_SENDER

    mail.send(Message(subject="Test Message from Tealdash", recipients=[email], body="Test message."))
