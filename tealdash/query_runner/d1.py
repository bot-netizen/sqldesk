...
from tealdash.utils.requests_session import requests_session as session

# Map Python types to Tealdash types
TYPES_MAP = {
    "str": TYPE_STRING,
    "int": TYPE_INTEGER,
    "float": TYPE_FLOAT,
    ...
    def run_query(self, query, user):
        try:
            ...
            for k, v in first_row.items():
                # Get the Python type name and map it to Tealdash type
                python_type = type(v).__name__
                column_type = TYPES_MAP.get(python_type, TYPE_STRING)
                ...
                columns.append({"name": k, "friendly_name": k, "type": column_type})
