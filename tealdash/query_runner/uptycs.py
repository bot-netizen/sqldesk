...
from tealdash.query_runner import BaseSQLQueryRunner, register
...
class Uptycs(BaseSQLQueryRunner):
    ...
    def transform_to_result_json(self, data):
        transformed_columns = []
        rows = []
    ...
    def api_call(self, sql):
        ...
        if response.status_code == 200:
            ...
            data = {}
            return data, error
        # if we get right status code then call transform_to_result_json
        data = self.transform_to_result_json(response_output)
        error = None
