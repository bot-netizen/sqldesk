...
def resolve_column_type(type_in_atsd):
    """
    Retrieve the corresponding column type
    :param type_in_atsd: `str`
    :return: column type constant
    """
    if isinstance(type_in_atsd, dict):
        column_type = types_map.get(type_in_atsd["base"])
    else:
        column_type = types_map.get(type_in_atsd)
    return column_type


def generate_rows_and_columns(csv_response):
    """
    Prepare rows and columns in the result format from ATSD csv response
    ...
    columns = [
        {
            ...
            "type": resolve_column_type(i["datatype"]),
            "name": i["name"],
        }
