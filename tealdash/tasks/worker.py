...
from tealdash import statsd_client

# HerokuWorker does not work on macOS: its fork handling assumes Linux signals.
...
class TealdashQueue(StatsdRecordingQueue, CancellableQueue):
    pass
...
class HardLimitingWorker(BaseWorker):
    ...
    in Tealdash (if we don't respect them, workers may be infinitely stuck and as a result,
...
class TealdashWorker(StatsdRecordingWorker, HardLimitingWorker):
    queue_class = TealdashQueue


Job = CancellableJob
Queue = TealdashQueue
Worker = TealdashWorker
