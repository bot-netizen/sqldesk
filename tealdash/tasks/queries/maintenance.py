...
from tealdash import models, redis_connection, settings, statsd_client
from tealdash.models.parameterized_query import (
    InvalidParameterError,
    QueryDetachedFromDataSourceError,
)
from tealdash.monitor import rq_job_ids
from tealdash.query_runner import NotSupported
from tealdash.tasks.failure_report import track_failure
from tealdash.utils import json_dumps, sentry
...
def refresh_queries():
    ...
    redis_connection.hset("tealdash:status", mapping=status)
...
def cleanup_query_results():
    ...
    # Deleting exactly the cap means there was more to delete than we were
    # allowed to. One run saying so is noise; every run saying so means the
    # table is growing faster than it is being emptied, which is invisible
    # otherwise until a disk fills up.
    if deleted_count >= settings.QUERY_RESULTS_CLEANUP_COUNT:
        logger.warning(
            "Query result cleanup hit its per-run cap of %d. Results are accumulating faster "
            "than they are being removed -- raise TEALDASH_QUERY_RESULTS_CLEANUP_COUNT.",
            settings.QUERY_RESULTS_CLEANUP_COUNT,
        )
