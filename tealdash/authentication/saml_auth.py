...
from tealdash.authentication.org_resolving import current_org
from tealdash.handlers.base import org_scoped_rule
from tealdash.utils import mustache_render
...
def idp_initiated(org_slug=None):
    if not current_org.get_setting("auth_saml_enabled"):
        ...
        return redirect(url_for("tealdash.index", org_slug=org_slug))
    ...
    try:
        ...
        return redirect(url_for("tealdash.login", org_slug=org_slug))
    ...
    group_names = get_group_names(authn_response.ava)
    if group_names is not None:
        user.update_group_assignments(group_names)

    url = url_for("tealdash.index", org_slug=org_slug)

    return redirect(url)


# Identity providers set up for Redash send group membership in an attribute
# named "RedashGroups", and changing that is a change to every customer's IdP
# configuration, not ours. Reading only the new name would not fail loudly: users
# would still log in, but land in the default group only and silently lose access
# to their data sources. Both names are accepted and the new one wins.
SAML_GROUP_ATTRIBUTES = ("TealdashGroups", "RedashGroups")


def get_group_names(ava):
    for attribute in SAML_GROUP_ATTRIBUTES:
        if attribute in ava:
            return ava.get(attribute)
    return None
