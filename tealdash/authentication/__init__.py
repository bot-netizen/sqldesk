...
from tealdash.authentication import jwt_auth
from tealdash.authentication.org_resolving import current_org
from tealdash.settings.organization import settings as org_settings
from tealdash.tasks import record_event
...
def get_login_url(external=False, next="/"):
    if settings.MULTI_ORG and current_org == None:  # noqa: E711
        ...
        login_url = url_for("tealdash.login", org_slug=current_org.slug, next=next, _external=external)
    else:
        login_url = url_for("tealdash.login", next=next, _external=external)
...
def logout_and_redirect_to_index():
    ...
    if settings.MULTI_ORG and current_org == None:  # noqa: E711
        ...
        index_url = url_for("tealdash.index", org_slug=current_org.slug, _external=False)
    else:
        index_url = url_for("tealdash.index", _external=False)

    return redirect(index_url)


def init_app(app):
    from tealdash.authentication import ldap_auth, remote_user_auth, saml_auth
    ...
    from tealdash.security import csrf
...
def get_next_path(unsafe_next_path):
    ...
    # This handles legitimate cases like "https://mytealdash.com/queries/5"
