...
from sqldesk import statsd_client
...
class SQLDeskQueue(StatsdRecordingQueue, CancellableQueue):
    pass
...
class HardLimitingWorker(BaseWorker):
    ...
    in SQLDesk (if we don't respect them, workers may be infinitely stuck and as a result,
...
class SQLDeskWorker(StatsdRecordingWorker, HardLimitingWorker):
    queue_class = SQLDeskQueue


Job = CancellableJob
Queue = SQLDeskQueue
Worker = SQLDeskWorker
