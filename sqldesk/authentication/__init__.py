...
from sqldesk.authentication import jwt_auth
from sqldesk.authentication.org_resolving import current_org
from sqldesk.settings.organization import settings as org_settings
from sqldesk.tasks import record_event
...
def get_login_url(external=False, next="/"):
    if settings.MULTI_ORG and current_org == None:  # noqa: E711
        ...
        login_url = url_for("sqldesk.login", org_slug=current_org.slug, next=next, _external=external)
    else:
        login_url = url_for("sqldesk.login", next=next, _external=external)
...
def logout_and_redirect_to_index():
    ...
    if settings.MULTI_ORG and current_org == None:  # noqa: E711
        ...
        index_url = url_for("sqldesk.index", org_slug=current_org.slug, _external=False)
    else:
        index_url = url_for("sqldesk.index", _external=False)

    return redirect(index_url)


def init_app(app):
    from sqldesk.authentication import ldap_auth, remote_user_auth, saml_auth
...
def get_next_path(unsafe_next_path):
    ...
    # This handles legitimate cases like "https://mysqldesk.com/queries/5"
