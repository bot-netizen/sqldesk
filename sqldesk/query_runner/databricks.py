...
from sqldesk import __version__, statsd_client
...
from sqldesk.settings import cast_int_or_default
...
class Databricks(BaseSQLQueryRunner):
    ...
    def _get_cursor(self):
        user_agent = "SQLDesk/{} (Databricks)".format(__version__.split("-")[0])
        ...
        try:
            ...
            if cursor.description is not None:
                ...
                if len(result_set) >= ROW_LIMIT and cursor.fetchone() is not None:
                    ...
                    statsd_client.incr("sqldesk.query_runner.databricks.truncated")
                    data["truncated"] = True
                error = None
            else:
                error = None
