...
@manager.command()
def check_settings():
    """Show the settings as SQLDesk sees them (useful for debugging)."""
...
def send_test_mail(email=None):
    ...
    from sqldesk import mail

    if email is None:
        email = settings.MAIL_DEFAULT_SENDER

    mail.send(Message(subject="Test Message from SQLDesk", recipients=[email], body="Test message."))
