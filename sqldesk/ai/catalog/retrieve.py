...
def find_tables(org, question, data_source=None, limit=DEFAULT_LIMIT):
    ...
    matched = base.filter(or_(*by_name, CatalogTable.id.in_(db.session.query(named_column.c.catalog_table_id))))
...
def context_for(org, question, data_source=None, limit=DEFAULT_LIMIT):
    ...
    for table in list(tables):
        for edge in neighbours_of(table):
            if len(chosen) >= limit:
                break
            if edge["table"] in chosen:
                continue
            neighbour = CatalogTable.query.filter(
                CatalogTable.data_source_id == table.data_source_id, CatalogTable.name == edge["table"]
            ).first()
            if neighbour is not None:
                chosen[neighbour.name] = neighbour

    ordered = sorted(chosen.values(), key=lambda t: -(t.usage_count or 0))
    return {
        "tables": [
            {
                "name": table.name,
